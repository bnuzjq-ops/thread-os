...
| 检查项 | 结果 | 证据 |
| --- | --- | --- |
| 内容源 | ✅ 12/12 | `content/scheduled/threads-20260720-shishen-dark-*.md` |
| 发布快照 | ✅ 12/12 | `publish-feed/posts/queue/` |
| 排期回执 | ✅ 12/12 `waiting` | `receipts/schedules/` |
| Cloudflare Workflow | ✅ 12/12 `Waiting` | 首篇实例睡眠至 `2026-07-20 21:00 +08:00` |
| Scheduler 健康检查 | ✅ | HTTP 200，返回 `ok` |
| GitHub 发布运行 | ⏳ 尚未触发 | 首篇尚未到 `21:00` |
| 真实发布回执 | ⏳ 尚无 | 尚未产生 `posted` / `platform_post_id` |

结论：排期和唤醒链当前正常，真实 Threads 发布尚未发生，需首篇到点后继续核验。
